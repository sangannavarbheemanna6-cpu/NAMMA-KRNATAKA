...
const CALENDAR = [
  [
    "🌾",
    "Paddy",
    "Nursery / transplanting / crop care",
    "ನಾಟಿ / ಬೆಳೆ ನಿರ್ವಹಣೆ",
  ],
  ...
  [
    "🌿",
    "Cotton",
    "Sowing / pest monitoring",
    "ಬಿತ್ತನೆ / ಕೀಟಗಳ ಮೇಲ್ವಿಚಾರಣೆ",
  ],
  ["🫘", "Tur", "Sowing / pest monitoring", "ಬಿತ್ತನೆ / ಕೀಟಗಳ ಮೇಲ್ವಿಚಾರಣೆ"],
  [
    "🥜",
    "Groundnut",
    "Sowing / disease monitoring",
    "ಬಿತ್ತನೆ / ರೋಗಗಳ ಮೇಲ್ವಿಚಾರಣೆ",
  ],
];
...
function FarmerHub() {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("langchange", changeLanguage);
  }, []);
  ...
  const filteredDistricts = useMemo(() => {
    ...
    return districts.filter((district: any) => {
      ...
      const cityMatch = (district.markets || []).some(
        (market: any) =>
          `${market.city?.en || market.city || ""} ${
            market.name?.en || ""
          } ${market.name?.kn || ""}`
            .toLowerCase()
            .includes(q)
  ...
  const cityMarkets = useMemo(() => {
    ...
    if (!selectedCity) {
      return currentDistrict.markets || [];
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (nearest) {
          ...
          setUserLocation(t(nearest.name, lang));
    ...
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">
            {lang === "kn"
              ? "ಜಿಲ್ಲೆ → ನಗರ / ಪಟ್ಟಣದ ಪ್ರಕಾರ"
              : "District → City / Town wise"}
          </p>
        </div>
                ...
                {alert ? (
                  ...
                  <div className="mt-3 rounded-xl bg-gray-50 p-3">
                    <p className="text-sm text-gray-600">
                      {lang === "kn"
                        ? "ಅಧಿಕೃತ IMD live alert data ಲಭ್ಯವಾದಾಗ ಇಲ್ಲಿ ತೋರಿಸಲಾಗುತ್ತದೆ."
                        : "Official IMD live alert data will be shown here when available."}
                    </p>
                  </div>
                )}
              </div>
      ...
      <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4">
        <p className="text-xs leading-5 text-blue-800">
          ℹ️{" "}
          {lang === "kn"
            ? "ಹವಾಮಾನ ಎಚ್ಚರಿಕೆಗಳನ್ನು ಅಧಿಕೃತ IMD ಮೂಲಗಳಿಂದ ಮಾತ್ರ ಪಡೆಯಬೇಕು. Fake ಅಥವಾ hard-coded current alerts ಬಳಸಲಾಗುವುದಿಲ್ಲ."
            : "Weather warnings should come only from official IMD sources. Fake or hard-coded current alerts are not used."}
        </p>
      </div>
    </section>
  );
  ...
  const Dashboard = () => (
    <div className="space-y-5">
      ...
      {/* WEATHER ALERTS - FARMER HUB DASHBOARD ಕೆಳಗಡೆ */}
      <WeatherAlerts />
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-blue-100 bg-blue-50 p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-blue-800">
          {lang === "kn"
            ? "ಹವಾಮಾನ ಎಚ್ಚರಿಕೆಗಳನ್ನು ಅಧಿಕೃತ IMD ಮೂಲಗಳಿಂದ ಮಾತ್ರ ಪಡೆಯಲಾಗುತ್ತದೆ."
            : "Weather warnings are intended to come only from official IMD sources."}
        </p>
      </div>
    </div>
  );
